...
{
	int counter = 0;
	for (int x = 0; x < worker->count; x++)
	{
		if (!worker->notifications[x]->isRed)
		{
			counter++;
		}
	}
...
Worker** workers = new Worker * [] {
	...
	NULL,NULL },
	new Worker{ new char[] {"leyla123"},new char[] {"Leyla"},new char[] {"IT"},
	NULL,NULL },
};
...
Employer** employers = new Employer * [] {
	...
	new AD * [] {
		...
		new AD{++global_id,new char[] {"Designer"},3000}
	},2 }
};
...
void SendNotificationsToAll(int ads_id, const char* username)
{
	...
	if (ads != nullptr)
	{
		for (int x = 0; x < ads->app_count; x++)
		{
			if (strcmp(ads->appliers[x]->username, username) == 1)
			{
				auto n = new Notification{ new char[] {"YOU ARE FAILED, you can try after 3 month!" }, __DATE__ };
				AddNotificationToWorker(ads->appliers[x], n);
			}
		}
	}
}
...
Employer* getEmployerByADId(Employer** employers, int ads_id, int e_count)
{
	for (size_t i = 0; i < e_count; i++)
	{
		for (size_t k = 0; k < employers[i]->ads[k]->app_count; k++)
...
int getIndexById(Employer** employers, int ads_id, int e_count)
{
...
void DeleteAdsById(Employer** employers, int ads_id, int e_count)
{
	...
	for (int i = index,i2=index+1; i2 < e->count; i++,i2++)
	{
		ads[i] = e->ads[i2];
	}
	e->ads = ads;
	...
	if (select == 'w') {
		...
		if (worker != nullptr) {
			...
			if (select == 1) {
				...
				worker->ads_id_count++;
				int* idptr = &id;
				worker->ads_id[worker->ads_id_count - 1] = idptr;
