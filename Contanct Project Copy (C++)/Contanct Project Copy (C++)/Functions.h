...
void ShowAllForGetch(Contact& contact, char* word) 
{
	...
	for (int x = 0; x < contact.count; x++)
	{
		...
		for (int y = 0; y < l; y++)
		{
			...
			||  isSuffix(contact.humans[x]->name, word) || isSuffix(contact.humans[x]->phonenumber, word))
			{
				ColorTextYellow();
				exists++;
				cout << "\n -------------------------------------------" << endl;
...
void AddNewHuman(Contact& contact, Human& human)
{
	...
	contact.count++;
	ColorTextGreen();
	cout << "\n New contact was added successfully!" << endl;
}
...
int GetHumanIndexName(Contact& contact, const char* name)
{
	int count = contact.count;
...
void DeleteByName(Contact& contact, char* name)
{
	...
	if (indexOFHuman >= 0)
	{
		...
		contact.count--;
		ColorTextGreen();
		cout << "\n \'" << name << "\' was deleted successfully!" << endl;
	}
	else
	{
		ColorTextRed();
...
int GetHumanIndexNameNumber(Contact& contact, const char* phonenumber)
{
	int count = contact.count;
	for (int x = 0; x < count; x++)
	{
...
void DeleteByPhoneNumber(Contact& contact, char* phonenumber)
{
	...
	int count = contact.count;
	auto newhumans = new Human * [count - 1]{};
	int indexOFHuman = GetHumanIndexNameNumber(contact, phonenumber);
	...
	if (indexOFHuman >= 0)
	{
		...
		newhumans = NULL;
		contact.count--;
		cout << "\n \'" << phonenumber << "\' was deleted successfully!" << endl;
	}
	else
	{
		cout << "\n There is no one with this number in the contact." << endl;
	}
}
